...
const InternshipRej = () => {
  ...
  const columns = [
    ...
    { 
      ...
      renderCell: (params) => (
        params.value ? 
        <a href={`http://localhost:3000/images/${params.value}`} target="_blank" rel="noopener noreferrer">View Report</a> 
        : 'No Report'
      ),
    },
    {
  ...
  const fetchData = async (option) => {
    try {
      const response = await fetch(`http://localhost:3000/rejectedApplications`);
      if (!response.ok) {
