...
const ListOfStudentsMappings = () => {
    ...
    const handleEdit = (row) => {
      ...
      const handleDeleteSubmit = async () => {
        try{
            const response = await axios.post('http://localhost:5001/api/ce/AddHm/DeleteAddOnHonorMinor',{courseCode,student,modeOfExemption})
      ...
      const fetchData = async (name) => {
        try {
          const response = await fetch(
            `http://localhost:5001/api/ce/AddHm/ListOfStudentMappings?name=${name}`
          );
          if (!response.ok) {
