...
const OneCreditStudentMappings = () => {
    ...
    const handleEditSubmit = async () => {
      try {
        const response = await axios.post(
          "http://localhost:5001/api/ce/oc/EditCourseList",
          {
            courseCode,
            courseName,
            student,
    ...
    const handleDeleteSubmit = async () => {
      try{
          const response = await axios.post('http://localhost:5001/api/ce/oc/DeletingOnecreditCompletion',{courseCode,student})
          console.log("Response :",response.data);
          if(response.status===200){
    ...
    const fetchData = async (name) => {
      try {
        const response = await fetch(
          `http://localhost:5001/api/ce/oc/SearchingOneCreditCourse?name=${name}`
        );
        if (!response.ok) {
