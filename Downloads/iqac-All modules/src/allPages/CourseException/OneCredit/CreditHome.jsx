      ...
      useEffect(()=> {
        const fetchExp = async () => {
          try{
            ...
            const resp1 = await fetch(`http://localhost:5001/api/ce/oc/OneCreditApprovalMembers`);
            if(!resp.ok){
            ...
            if(Data[0].approval_status===0){
              const member = Data1[0].members
              setStatus(member);
              setCls(0);
            }else if(Data[0].approval_status===1){
              const member = Data1[1].members
              setStatus(member);
              setCls(1);
            }
            else if(Data[0].approval_status===2){
              const member = Data1[2].members
              setStatus(member);
              setCls(2);
            }
            else if(Data[0].approval_status===3){
              setStatus("Approved")
              setCls(3);
            }
            else{
  ...
  return (
    <div className='creditHomeMain' >
        ...
        <div className='createDiv' >
            ...
            <div className='expBtnsReasons'><div><button className={data.length>=3?"CourseBtnActive":"CourseBtn"} onClick={() => setShowModal(true)} disabled={data.length>=3?false:true} >Course Exception</button></div>
            {data.length<3 ? <div className='btnReason' onClick={handleNotification} ><CircleNotificationsIcon/></div>:""}</div>}
        </div>
        </div>
      ...
      <Modal open={expModal} onClose={() => setExpModal(false)}>
        <Box sx={style}>
              ...
              <div className='oneCreditModal'>
                ...
                <div style={{marginTop:"10px"}} > <StepperWithContent status={cls} data={membersData} rejection={rejectionStatus} /> </div>
