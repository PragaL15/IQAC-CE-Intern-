...
const fetchUsers = async () => {
  try {
    const typePromise = axios.get("http://localhost:5001/api/ce/oc/platform");
    const validPromise = axios.get("http://localhost:5001/api/ce/oc/courseExpValidation");
    const decisionStatusPromise = axios.get(`http://localhost:5001/api/ce/oc/AllActiveApplications?student=${student}`);
    const respPromise = axios.get(`http://localhost:5001/api/ce/oc/ApprovedStatusAll?student=${student}`);
    const yearPromise = axios.get("http://localhost:5001/api/ce/AvailableAcademicYears");
    ...
    const [typeResult, validResult, decisionStatusResult, respResult, yearResult] = results;
    ...
    if (decisionStatusResult.status === 'fulfilled') {
      ...
      console.error("Error fetching decision status:", decisionStatusResult.reason);
    }
...
//getting details from backend for default details 
...
useEffect(() => {
    ...
    if (course) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5001/api/ce/oc/platform/excemption?id=${selectedCourse}`
        );
        const resp = await axios.get(
          `http://localhost:5001/api/ce/oc/courselist?platform=${selectedCourse}&student=${student}`
        );
    ...
    try {
      if (
        ...
        const response = await axios.post(
          "http://localhost:5001/api/ce/oc/onlineApply/create",
          formData,
          {
            headers: {
  ...
  const handleRestrict = async () => {
    // const resp = await axios.get(`http://localhost:5001/api/ce/oc/ApprovedStatusAll?student=${student}`)
  ...
  const handleAcademicYear = async (selectedOption)=>{
    ...
    setSemesterOptions([]);
    try {
      const response = await axios.get(`http://localhost:5001/api/ce/AvailableSemester?id=${selectedOption.value}`);
