...
const InternshipCompanies = () => {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await axios.post(
        "http://localhost:5001/api/ce/in/InternCompanyEdit",
        {
          companyName,
          companyAddress,
          companyPhoneNumber,
  ...
  const handleDeleteSubmit = async () => {
    try{
        const response = await axios.post('http://localhost:5001/api/ce/in/InternCompanyDelete',{selectedCompanyId})
        console.log("Response :",response.data);
        if(response.status===200){
  ...
  const fetchData = async (name) => {
    try {
      const response = await fetch(
        `http://localhost:5001/api/ce/in/InternCompanySearch?name=${name}`
      );
      if (!response.ok) {
