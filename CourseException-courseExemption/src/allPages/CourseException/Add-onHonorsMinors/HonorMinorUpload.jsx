...
const HonorMinorUpload = () => {
  ...
  useEffect(() => {
    ...
    const fetchAcademicYear = async () => {
      const yearPromise = await axios.get(`${apiBaseUrl}/api/ce/AvailableAcademicYears`);
  ...
  const AcademicYearList = academicYearData.map((year)=>({
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          const response = await axios.post(`${apiBaseUrl}/api/ce/AddHm/HonorMinorSingleUpload`, formData);
          if (response.status === 200) {
